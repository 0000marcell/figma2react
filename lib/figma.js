...
const createComponent = (component, imgMap, componentMap) => {
  ...
  let doc = '';
  print(`class ${name} extends Component {`, '');
  print(`  render() {`, '');
  print(`    return (`, '');
    ...
    if (node.id !== component.id && node.name.charAt(0) === '#') {
      ...
      if (content != null) {
        if (node.name.charAt(0) === '$') {
          ...
          props.push({ name: varName, type: [PROP_TYPES.NUMBER, PROP_TYPES.STRING] });
  ...
  componentMap[component.id] = {instance, name, doc, props};
}

module.exports = {createComponent, colorString, PROP_TYPES}
